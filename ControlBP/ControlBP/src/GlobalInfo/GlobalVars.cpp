...
void initialise_competition_data(position start_position)
{
    ...
    run_status.bot_position.currently_facing = start_position.currently_facing; 
...
void initialise_competition_data()
{
    ...
    if (digitalRead(THANOS_v_METHANOS_SWITCH) == METHANOS) {
        my_position = {METHANOS_START, METHANOS_GAUNTLET, FACING_NEXT}; //, 0, 0, 0}; // i.e. current position is METHANOS_START, next position is METHANOS_GAUNTLET, we're 0 of the way there
    } else {
        my_position = {THANOS_START, THANOS_GAUNTLET, FACING_NEXT}; //, 0, 0, 0}; // i.e. current position is METHANOS_START, next position is METHANOS_GAUNTLET, we're 0 of the way there
    }
